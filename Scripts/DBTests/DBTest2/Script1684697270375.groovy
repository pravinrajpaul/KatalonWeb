...
actorDataS = CustomKeywords.'com.katalon.plugin.keyword.connection.DatabaseKeywords.executeQuery'(connection, 'select * from rentalsch.actor order by actor_id asc;')

actorDataT = CustomKeywords.'com.katalon.plugin.keyword.connection.DatabaseKeywords.executeQuery'(connection, 'select * from rentalsch.actor order by actor_id asc;')
...
if (actorDataSRowCount == actorDataTRowCount && actorDataSColumnCount == actorDataTColumnCount) {
	...
	for(int i=1; i<=actorDataSRowCount; i++) {
		for (int j=1; j<=actorDataSColumnCount; j++) {
			sValue = CustomKeywords.'com.katalon.plugin.keyword.connection.ResultSetKeywords.getSingleCellValue'(actorDataS, i, j)
			tValue = CustomKeywords.'com.katalon.plugin.keyword.connection.ResultSetKeywords.getSingleCellValue'(actorDataT, i, j)
			
			if (sValue != tValue) {
				fail.add("Source value in cell " + i + "," + j + ":" + sValue + "; mismatch target value :" + tValue)
			}
		}
	}
}
